...
matrix_model <- function(data, initialPop, timesteps, corrMatrix = NULL, colNames = NULL, firstElement = "reproduction"){
  ...
  if (is.null(corrMatrix)==T) {
    ...
    elements$noise <- colored_multi_rnorm(100, elements$mean.trans, elements$sd.trans, elements$autocorrelation, corrMatrix) %>%
      split(rep(1:ncol(.), each = nrow(.)))
    elements %>% mutate(natural.noise = ifelse(dist == "log", list(exp(noise)), list(plogis(noise)))) -> elements
  } else {
