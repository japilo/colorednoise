...
blue <- colored_noise(timesteps = 100, mu = 0.5, sigma = 0.2, phi = -0.5)
red <- colored_noise(timesteps = 100, mu = 0.5, sigma = 0.2, phi = 0.5)
...
```{r generate noise}
red <- colored_noise(timesteps = 100, mu = 0.3, sigma = 1.2, phi = 0.5)
red[1:10]
blue <- colored_noise(timesteps = 100, mu = 0.3, sigma = 1.2, phi = -0.5)
blue[1:10]
ggplot(data = NULL, aes(x = c(1:100), y = blue)) + geom_line(color="blue") + theme_minimal() + theme(axis.title = element_blank()) + ggtitle("Blue Noise")
ggplot(data = NULL, aes(x = c(1:100), y = red)) + geom_line(color="red") + theme_minimal() + theme(axis.title = element_blank()) + ggtitle("Red Noise")
```
...
raw_sims <- cross_df(list(timesteps = 20, phi = c(-0.5, 0, 0.5), mu = 0.3, sigma = c(0.5, 0.7, 0.9, 1.1, 1.3, 1.5))) %>% rerun(.n = 30, pmap(., colored_noise))
labels <- raw_sims %>% map(1) %>% bind_rows()
noise <- raw_sims %>% map(2) %>% flatten()
estimates <- data.frame(mean = map_dbl(noise, mean), sd = map_dbl(noise, sd), autocorrelation = map_dbl(noise, autocorrelation))
